...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.5, epsilon_threshold=0.005, a=0.5, experiment="na"):
    ...
    def choose_action(self, state):
        ...
        if self.epsilon > 0.01 and self.epsilon > random.random():
            ...
            actions_ = [action_ for (action_, value_) in self.Q[self.key_state(state)].iteritems() if value_ == max_value]
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            ...
            self.Q[self.key_state(state)][action] = (1 - self.alpha) * s_a + self.alpha * reward
        return
    ...
    def key_state(self, state):
        ...
        return "{0}_LI:{1}_O:{2}_R:{3}_L:{4}".format(state[0], state[1]['light'], state[1]['oncoming'], state[1]['right'], state[1]['left'])
...
def run():
    ...
    agent = env.create_agent(LearningAgent, learning=True, epsilon=1.0, alpha=0.007, epsilon_threshold=0.0003,
                                            experiment="na", a=0.003)
